...
// Version 0.1 vom 04.02.2015/ms fue HM Ottmarsheim
...
byte ccu[] = { 192, 168, 11, 220 };                   //das ist die IP der CCU                                    <<user-eingabe<<
byte mac[] = { 0xCC, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC };  //muss nur bei mehreren homeduinos geändert werden !        <<user-eingabe<<
byte homeduino[] = { 192, 168, 11, 221 };             //das ist die feste IP des Homeduino, wenn DHCP versagt     <<user-eingabe<<
...
byte iomodus_D[10] = {0, 0,
                      ...
                      0,   //D4  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall;                  <<user-eingabe<<
                      0,   //D5  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall;                  <<user-eingabe<<
                      5,   //D6  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall; "5" = digital_out <<user-eingabe<<
                      5,   //D7  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall; "5" = digital_out <<user-eingabe<<
                     -1,   //D8  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall; "5" = digital_out <<user-eingabe<<
                     -1    //D9  :"0" = digital_in; "1" = 1wire; "3" = SHTxx; "4" = Ultraschall; "5" = digital_out <<user-eingabe<<
                     };
...
byte iomodus_A[6] = {10,   //A0  :"0" = digital_in; "10" = analogin; "11" = NTC; "1" = 1wire;                     <<user-eingabe<<
                     ...
                     10,   //A3  :"0" = digital_in; "10" = analogin; "11" = NTC; "1" = 1wire;                     <<user-eingabe<<
                     10,   //A4  :"0" = digital_in; "10" = analogin; "11" = NTC; "1" = 1wire;                     <<user-eingabe<<
                     10    //A5  :"0" = digital_in; "10" = analogin; "11" = NTC; "1" = 1wire;                     <<user-eingabe<<
                    };
//hier werden Sensoren am I2C-Eingang aktiviert
byte iomodus_baro  = 0;   //"0" = N.C.;  "1" = BMP180                     <<user-eingabe<<
byte iomodus_lux  = 0;   //"0" = N.C.; "1" = BH1750;                      <<user-eingabe<<



//hier werden die zuletzt gesendeten sytemvariablen gespeichert
boolean last_digitalin_value_D[10];
boolean last_digitalout_value_D[10];
...
unsigned long delta_tx = 20000;  //in ms, minimaler Abstand der Telegramme an die CCU
...
{ t.update();
  ...
  { complete_loop = 1; next_full_loop = millis() + delta_time;
    if (next_full_loop < millis()) {
      complete_loop = 0; //wichtig wegen Zahlensprung von millis() alle 50 Tage
    }
  }
  if (millis() > next_tx) { //warten bis time > next_tx oder timeout
    next_tx = millis() + delta_tx;
    //read_sysvar();
  }
}
//##############################################################
// Unterprogramme fuer Zeitscheiben
void doAll1Sek(void* context) {
  ...
  pinMode(i, OUTPUT);
  digitalWrite(i, last_digitalout_value_D[i]);
  last_digitalout_value_D[i] = !last_digitalout_value_D[i];
}
...
void read_sysvar()
{
    ...
    { if (iomodus_D[i] == 0)
      ...
      { pinMode(i, OUTPUT);
        ...
        Serial.print(i);
        Serial.print(" = ");
        Serial.println(last_digitalout_value_D[i]);
        last_digitalout_value_D[i] = !last_digitalout_value_D[i];
      }
    }
    //**************************************************************************************************
...
#define DS18B20_ID 0x28
        ...
        temp_tur = 1000.0;
        if (!ds.search(addr)) {
          ds.reset_search();  //find a device
          temp_tur = -1000.0;
        }
        if ((OneWire::crc8( addr, 7) != addr[7]) && (temp_tur > -1000.0)) {
          temp_tur = -1000.0;
        }
        ...
        { ds.reset();
          ...
          for ( int k = 0; k < 9; k++) {
            data[k] = ds.read();  // Receive 9 bytes
          }
...
#define DS18B20_ID 0x28
    ...
    { if (pressure.begin()) {
      ...
      if (status) {
        delay(status);  //messung T
        status = pressure.getTemperature(T);
      }
      if (status) {
        status = pressure.startPressure(3); // //messung P mit resolution 0 bis 3
      }
      if (status) {
        delay(status);
      ...
      if (status) {
        p0 = pressure.sealevel(P, ALTITUDE); // umrechnung auf N.N.
      }
